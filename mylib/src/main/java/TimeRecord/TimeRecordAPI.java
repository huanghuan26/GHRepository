...
public class TimeRecordAPI
{
    ...
    public enum enMsgType {
                                                        /*↓对应结构体↓*/
        MSG_TerminalRegister,   //终端注册平台回应   无对应结构体，参数nValue为应答结果 1：成功 2：失败
        MSG_CoachLogin,         //教练登录平台回应,     CoachLogin_S
        MSG_CoachLogout,        //教练登出平台回应      CoachLogout_S
        MSG_StudentLogin,       //学员登陆平台回应      StudentLogin_S
   ...
   // public class SuperStruct  {}
    ...
    public  int TR_Init(MsgCallBack CB)
    {
        if(Pri != null)
        {
            return 1;
        }
        Pri = new Primary(this);
        this.CB = CB;
...
public int TR_StudentLogin(String sStudentID, String sStudentsCoach, int nTrainObject)
{
    if(Pri == null)
    {
        return -3;
    ...
    int rt = Pri.UI_StudentLogin(sStudentID,sStudentsCoach,nTrainObject);
    return rt;
}
